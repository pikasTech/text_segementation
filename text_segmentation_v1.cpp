...
void draw_projection(vector<int>& pos, int mode) {
    vector<int>::iterator max =
        std::max_element(std::begin(pos), std::end(pos));  //求最大值
    if (mode == H_PROJECT) {
        int height = pos.size();
        int width = *max;
...
int GetPeekRange(vector<int>& vertical_pos,
                 vector<char_range_t>& peek_range,
                 int min_thresh = 2,
                 int min_range = 10) {
    int begin = 0;
    int end = 0;
...
//切割字符
int CutChar(Mat& img,
            const vector<char_range_t>& v_peek_range,
            const vector<char_range_t>& h_peek_range,
            vector<Mat>& chars_set) {
    static int count = 0;
    int norm_width =
        img.rows;  //因为汉字都是类正方形的，所以我们定了norm_width就是汉字的高度
    ...
    for (int i = 0; i < v_peek_range.size(); i++) {
        ...
        // if (char_gap <= (int)(norm_width*1.2) && char_gap >=
        // (int)(norm_width*0.8))
        {
            int x =
                v_peek_range[i].begin - 2 > 0 ? v_peek_range[i].begin - 2 : 0;
...
vector<Mat> CutSingleChar(Mat& img) {
    ...
    for (int i = 0; i < h_peek_range.size(); i++) {
        Mat line =
            cut_one_line(img, h_peek_range[i].begin, h_peek_range[i].end);
        lines_set.push_back(line);
        // Mat line_show = show(Rect(0, h_peek_range[i].begin, show.cols,
        // h_peek_range[i].end - h_peek_range[i].begin));
